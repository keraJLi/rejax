...
def train_iteration(config, ts):
    ...
    ts, batch = collect_transitions(config, ts, uniform=uniform)
...
def collect_transitions(config, ts, uniform=False):
    # Sample actions
    ...
    def sample_policy(rng):
        ...
        noise = config.exploration_noise * jax.random.normal(rng_noise, actions.shape)
    ...
    minibatch = Minibatch(
        obs=ts.last_obs, action=actions, reward=rewards, next_obs=next_obs, done=dones
    )
    ts = ts.replace(
        last_obs=next_obs,
        env_state=env_state,
